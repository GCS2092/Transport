...
function iconForRow(c: SettingsContactRow) {
  const l = c.label.toLowerCase()
  if (l.includes('whatsapp')) return <WhatsSvg />
  if (l.includes('mail') || l.includes('email') || l.includes('e-mail')) return <MailSvg />
  if (l.includes('adresse') || l.includes('siège')) return <MapSvg />
  return <PhoneSvg />
}
...
export default function ContactPage() {
  ...
  const [contacts, setContacts] = useState<SettingsContactRow[]>([])
  const [faqs, setFaqs] = useState<SettingsFaqRow[]>([])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const [ct, fq] = await Promise.all([
          fetchPublicContacts(),
          fetchPublicFaqs(lang === 'en' ? 'en' : 'fr'),
        ])
        if (!cancelled) {
          setContacts(ct)
          setFaqs(fq)
  ...
  const displayContacts = contactPageRows(contacts)
  const faqItems =
    faqs.length > 0 ? faqs.map(f => ({ q: f.question, a: f.answer })) : c.faq
  ...
  return (
    <div className="bg-[var(--bg)] pb-6">
      <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        {loading ? (
          <div className="bg-[var(--primary)] rounded-2xl p-8 mb-4 animate-pulse h-48" />
        ) : (
          <>
            <div className="bg-[var(--primary)] rounded-2xl p-5 mb-4 space-y-4">
              {displayContacts.map(item => (
                <a key={item.id} href={item.href || undefined} className="flex items-center gap-4">
        ...
        <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden mb-4">
          ...
          <div className="divide-y divide-[var(--border)]">
            {loading ? (
              <div className="px-5 py-8 text-sm text-[var(--muted)]">…</div>
            ) : (
              faqItems.map((item, i) => (
                <details key={`${i}-${item.q.slice(0, 20)}`} className="group">
